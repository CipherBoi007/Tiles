...
async function main() {
    ...
    await prisma_1.default.tile.deleteMany({});
    await prisma_1.default.subCategory.deleteMany({});
    await prisma_1.default.category.deleteMany({});
    await prisma_1.default.catalogue.deleteMany({});
    await prisma_1.default.enquiry.deleteMany({});
    await prisma_1.default.activity.deleteMany({});
    // Seed Categories
    const categories = [];
    for (const cat of categoriesData) {
        const created = await prisma_1.default.category.create({ data: cat });
    ...
    const existingAdmin = await prisma_1.default.adminUser.findUnique({ where: { email: 'admin@admin.com' } });
    if (!existingAdmin) {
    ...
    await prisma_1.default.activity.createMany({
        data: [
